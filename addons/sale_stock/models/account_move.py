...
class AccountMoveLine(models.Model):
    ...
    def _stock_account_get_anglo_saxon_price_unit(self):
        ...
        if so_line:
            ...
            posted_invoice_lines = so_line.invoice_lines.filtered(
                lambda l: l.move_id.state == 'posted' and bool(l.move_id.reversed_entry_id) == is_line_reversing)
            if posted_invoice_lines:
                posted_reversed_invoice_lines = so_line.invoice_lines.filtered(
                    lambda l: l.move_id.state == 'posted' and bool(l.move_id.reversed_entry_id))
                qty_invoiced = sum([x.product_uom_id._compute_quantity(
                    x.quantity, x.product_id.uom_id) for x in posted_invoice_lines]) - sum(
                    [x.product_uom_id._compute_quantity(
                        x.quantity, x.product_id.uom_id) for x in posted_reversed_invoice_lines])
            else:
                qty_invoiced = sum([x.product_uom_id._compute_quantity(
                    x.quantity, x.product_id.uom_id) for x in posted_invoice_lines])

            average_price_unit = self.product_id.with_context(
                force_company=self.company_id.id, is_returned=is_line_reversing)._compute_average_price(
                qty_invoiced, qty_to_invoice, so_line.move_ids)
            price_unit = average_price_unit or price_unit
            price_unit = self.product_id.uom_id.with_context(
                force_company=self.company_id.id)._compute_price(price_unit, self.product_uom_id)
        return price_unit
